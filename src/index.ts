...
async function oauthAuthorize(req: Request, env: Env): Promise<Response> {
  ...
  if (req.method === 'GET') return authorizeForm(req, u.searchParams.has('error'));

  if (req.method === 'POST') {
    let password = '';
    try { const fd = await req.formData(); password = (fd.get('password') as string) ?? ''; }
...
async function oauthToken(req: Request, env: Env): Promise<Response> {
  ...
  try { params = new URLSearchParams(await req.text()); }
  ...
  if (grantType === 'authorization_code') {
    ...
    if (codeChallenge && verifier && !(await verifyPkce(verifier, codeChallenge))) {
...
class SimpleMCPServer {
  ...
  async handleRequest(request: any) {
    ...
    try {
      switch (method) {
        case 'initialize':
          return { jsonrpc: '2.0', id, result: { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'youtube-transcript-remote', version: '1.0.0' } } };
        case 'tools/list':
          return { jsonrpc: '2.0', id, result: { tools: [{ name: 'get_transcript', description: 'Extract transcript from YouTube video URL', inputSchema: { type: 'object', properties: { url: { type: 'string', description: 'YouTube video URL (any format)' }, language: { type: 'string', description: "Language code e.g. 'en', 'es'. Defaults to 'en'." } }, required: ['url'] } }] } };
        case 'tools/call': {
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/') {
      return new Response(JSON.stringify({ name: 'YouTube Transcript Remote MCP Server', version: '1.0.0', endpoints: { sse: '/sse', mcp: '/mcp' }, tools: ['get_transcript'], status: 'ready' }), { headers: { 'Content-Type': 'application/json', ...CORS } });
    }
